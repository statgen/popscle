...
int32_t cmdCramFreemuxlet(int32_t argc, char **argv) {
    ...
    BEGIN_LONG_PARAMS(longParameters)
                    ...
                    LONG_STRING_PARAM("init-cluster", &initClusterFile,
                                      "Input file containing the initial cluster information")
                    LONG_STRING_PARAM("init-dist", &initDistFile,
                      "Input file to reuse genetic distance information from auxFiles")
...
//    LONG_MULTI_DOUBLE_PARAM("alpha",&gridAlpha, "Grid of alpha to search for (default is 0, 0.5)")
                    ...
                    LONG_PARAM("aux-files", &auxFiles, "Turn on writing auxiliary output files")
                    LONG_INT_PARAM("verbose", &globalVerbosityThreshold,
    ...
    */
    ...
    if (!initClusterFile.empty()) {
        ...
        while (tsv_clustf.read_line() > 0) {
            ...
            if (iclust >= 0) {
                if (iclust >= nSamples)
                    error("ERROR: --nsample %d parameter was set. The cluster ID must be between 0 to %d, or use negative values to not assign initial cluster (not implemented yet)",
                          nSamples, nSamples - 1);
    ...
    for (int32_t i = 0; i < scl.nbcs; ++i) {
        ...
        hprintf(wmix, "%d\t%s\t%d\t%d\t%.2lf\t%.2lf\t%.2lf\t%.4lf\n", si, scl.bcs[si].c_str(), nSNPs[i], nReads[i],
                llk0, llk2, llk0 - llk2, (llk0 - llk2) / nSNPs[i]);
    }
    hts_close(wmix);
    ...
    if(!initDistFile.empty())
    {
        htsFile * rdist = hts_open((initDistFile + ".ldist.gz").c_str(), "rz");
        if(!rdist)
        {
            numerror("Open file %s failed!",(initDistFile + ".ldist.gz").c_str());
        } else
            notice("Reading pairwise genetic distance matrix..");
        ...
        int32_t readmin(0);
        kstring_t str = {0,0,0};
        hts_getline(rdist, KS_SEP_LINE, &str);//eat header
        while ( hts_getline(rdist, KS_SEP_LINE, &str) >= 0 )
        {
            if (v % 1000000 == 0)
                notice("Processing %d pair droplets...", v);
            std::stringstream ss(str.s);
            int32_t si(0),sj(0);
            ss>>si>>sj;
            dropD &dd = (si > sj) ? dropDs[si][sj] : dropDs[sj][si];
            ss>>dd.nsnps>>dd.nread1>>dd.nread2>>readmin>>dd.llk0>>dd.llk2;
            //std::cerr<<si<<"\t"<<sj<<"\t"<<dd.nsnps<<std::endl;
            v++;
        }
        hts_close(rdist);
...
//                                dd.nread1,
//                                dd.nread2, dd.nread1 > dd.nread2 ? dd.nread2 : dd.nread1, dd.llk0, dd.llk2,
//                                dd.llk2 - dd.llk0, (dd.llk2 - dd.llk0) / (dd.nsnps + 1e-6));
//                    }
...
//        if (auxFiles) hts_close(wdist);
        ...
        for (int32_t v = 0; v < scl.nsnps; ++v) {
            if (!snp_cell_plps[v].empty()) {
                if (v % 10000 == 0)
                    notice("Processing %d variants at %s:%d..", v, scl.rid2chr[scl.snps[v].rid].c_str(),
                           scl.snps[v].pos);
    ...
    //merge droplets belong to the same cluster
    ...
    if (auxFiles) {
        ...
        hprintf(vc0,
                "##FORMAT=<ID=GP,Number=G,Type=Float,Description=\"Posterior probability using pooled allele frequencies\">\n");
        ...
        for (int32_t v = 0; v < scl.nsnps; ++v) {
            ...
            hprintf(vc0, "%s\t%d\t.\t%c\t%c\t.\tPASS\tAF=%.5lf\tGT:GQ:DP:AD:PL:GP", scl.rid2chr[s.rid].c_str(), s.pos,
                    s.ref, s.alt, s.af);
            ...
            for (int32_t i = 0; i < nSamples; ++i) {
                ...
                hprintf(vc0, "\t%d/%d:%d:%d:%d,%d:%d,%d,%d:%.3lg,%.3lg,%.3lg", bestG == 2 ? 1 : 0, bestG > 0 ? 1 : 0,
                        gq, sdp.nreads, sdp.nref, sdp.nalt, pls[0], pls[1], pls[2], pps[0], pps[1], pps[2]);
            }
            hprintf(vc0, "\n");
        }
        hts_close(vc0);
...
//            }
    ...
    for (int32_t v = 0; v < scl.nsnps; ++v) {
        if (!snps_observed[v]) continue;
        sc_snp_t &s = scl.snps[v];
        hprintf(vc1, "%s\t%d\t.\t%c\t%c\t.\tPASS\tAF=%.5lf\tGT:GQ:DP:AD:PL:GP", scl.rid2chr[s.rid].c_str(), s.pos,
                s.ref, s.alt, s.af);
        double gps[3] = {(1. - s.af) * (1. - s.af), 2. * s.af * (1. - s.af), s.af * s.af};
        double pps[3], sumPP;
        int32_t pls[3];
        ...
        for (int32_t i = 0; i < nSamples; ++i) {
            ...
            if (gq > 255) gq = 255;
            hprintf(vc1, "\t%d/%d:%d:%d:%d,%d:%d,%d,%d:%.3lg,%.3lg,%.3lg", bestG == 2 ? 1 : 0, bestG > 0 ? 1 : 0, gq,
                    sdp.nreads, sdp.nref, sdp.nalt, pls[0], pls[1], pls[2], pps[0], pps[1], pps[2]);
        }
        hprintf(vc1, "\n");
    }
    hts_close(vc1);
    ...
    for (int32_t i = 0; i < scl.nbcs; ++i) {
        hprintf(wc1,
                "%d\t%s\t%d\t%d\t%s\t%d,%d\t%.2lf\t%d,%d\t%.2lf\t%.2lf\t%.5lf\t%.2lg\t%d\t%.2lf\t%d\t%.2lf\t%.5lf\t%d,%d\t%.2lf\t%.2lf\n",
                i, scl.bcs[i].c_str(), nSNPs[i], nReads[i], (types[i] == 2) ? "AMB" : ((types[i] == 0) ? "SNG" : "DBL"),
                jBests[i], kBests[i], bestLLKs[i], jNexts[i], kNexts[i], nextLLKs[i], bestLLKs[i] - nextLLKs[i],
                bestPPs[i], sngPPs[i], sBests[i], sngBestLLKs[i], sNexts[i], sngNextLLKs[i], sngOnlyPPs[i], dBest1s[i],
                dBest2s[i], dblBestLLKs[i], sngBestLLKs[i] - dblBestLLKs[i]);
    }
    hts_close(wc1);
