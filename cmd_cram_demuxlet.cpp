...
int32_t cmdCramDemuxlet(int32_t argc, char **argv) {
    ...
    BEGIN_LONG_PARAMS(longParameters)
                    ...
                    LONG_STRING_PARAM("sam", &sr.sam_file_name,
                                      "Input SAM/BAM/CRAM file. Must be sorted by coordinates and indexed")
                    LONG_STRING_PARAM("tag-group", &tagGroup,
                    ...
                    LONG_INT_PARAM("min-MQ", &sr.filt.minMQ,
                    ...
                    LONG_STRING_PARAM("group-list", &groupList,
    ...
    if (!plpPrefix.empty()) { // read from pileup
        ...
        if (tagGroup.empty()) { // do nothing
            ...
            error("[E:%s:%d %s] Cannot recognize group tag %s. It is suppose to be a length 2 string", __FILE__,
                  __LINE__, __FUNCTION__, tagGroup.c_str());
        }
        ...
        if (tagUMI.empty()) { // do nothing
            ...
            error("[E:%s:%d %s] Cannot recognize UMI tag %s. It is suppose to be a length 2 string", __FILE__, __LINE__,
                  __FUNCTION__, tagUMI.c_str());
        }
        ...
        if (!vr.parse_posteriors(vr.cdr.hdr, vr.cursor(), field.c_str()))
            error("[E:%s] Cannot parse posterior probability at %s:%d", __PRETTY_FUNCTION__,
                  bcf_hdr_id2name(vr.cdr.hdr, vr.cursor()->rid), vr.cursor()->pos + 1);
        ...
        for (int32_t i = 0; i < ntids; ++i) {
            ...
            if (rid >= 0) {
                if (prevrid >= rid) {
                    ...
                    error("[E:%s] Your VCF/BCF files and SAM/BAM/CRAM files have different ordering of chromosomes. SAM/BAM/CRAM file has %s before %s, but VCF/BCF file has %s after %s",
                          __PRETTY_FUNCTION__, prevchrom, chrom, prevchrom, chrom);
                }
                rid2tids[rid] = i;
                tid2rids[i] = rid;
                prevrid = rid;
        ...
        int32_t snpid = scl.add_snp(vr.cursor()->rid, vr.cursor()->pos + 1, vr.cursor()->d.allele[0][0],
                                    vr.cursor()->d.allele[1][0], vr.get_af(1), gps);
        ...
        while (sr.read()) { // read SAM file
            ...
            int32_t n_cleared = vr.clear_buffer_before(bcf_hdr_id2name(vr.cdr.hdr, vr.cursor()->rid),
                                                       sr.cursor()->core.pos);
            ...
            // add new snps
            while ((!vr.eof) &&
                   ((vr.cursor()->rid < tid2rid) || ((vr.cursor()->rid == tid2rid) && (vr.cursor()->pos < endpos)))) {
                if (vr.read()) {
                    ...
                    if (genoErrorCoeffR2 > 0) { // look for R2 INFO field
                        if ((bcf_get_info_float(vr.cdr.hdr, vr.cursor(), r2Info.c_str(), &r2flts, &n_r2flts) < 0) ||
                            (n_r2flts != 1)) {
                            error("Cannot extract %s (1 float value) from INFO field at %s:%d. Cannot use --geno-error-coeff",
                                  r2Info.c_str(), bcf_hdr_id2name(vr.cdr.hdr, vr.cursor()->rid), vr.cursor()->pos + 1);
                        }
                    ...
                    snpid = scl.add_snp(vr.cursor()->rid, vr.cursor()->pos + 1, vr.cursor()->d.allele[0][0],
                                        vr.cursor()->d.allele[1][0], vr.get_af(1), gps);
                    snpids.push_back(snpid);
                } else {
            ...
            if (tagGroup.empty()) {
                ...
                if ((bcd != NULL) && (*bcd == 'Z')) {
                    ...
                    if (n_warning_no_gtag < 10) {
                        notice("WARNING: Cannot find Droplet/Cell tag %s from %d-th read %s at %s:%d-%d. Treating all of them as a single group",
                               tagUMI.c_str(), sr.n_read, bam_get_qname(sr.cursor()),
                               bam_get_chrom(sr.hdr, sr.cursor()), sr.cursor()->core.pos, bam_endpos(sr.cursor()));
            ...
            if (tagUMI.empty()) {
                ...
                if ((umi != NULL) && (*umi == 'Z')) {
                    ...
                    if (n_warning_no_utag < 10) {
                        notice("WARNING: Cannot find UMI tag %s from %d-th read %s at %s:%d-%d. Treating all of them as a single UMI",
                               tagUMI.c_str(), sr.n_read, bam_get_qname(sr.cursor()),
                               bam_get_chrom(sr.hdr, sr.cursor()), sr.cursor()->core.pos, bam_endpos(sr.cursor()));
            ...
            for (int32_t i = ibeg; i < ibeg + vr.nbuf; ++i) {
                bam1_t *b = sr.cursor();
                bam_get_base_and_qual_and_read_and_qual(b, (uint32_t) scl.snps[i].pos - 1, base, qual, rpos, &readseq,
                                                        &readqual);
        ...
        notice("Total number of read-QC-passed reads : %d ",
               sr.n_read - sr.n_skip); //, nReadsN + nReadsUnique + nReadsLQ + nReadsPass);
        ...
        if (minTotalReads + minUniqReads + minCoveredSNPs < 0) {
            for (int32_t i = 0; i < scl.nbcs; ++i) {
                if ((scl.cell_totl_reads[i] < minTotalReads) || (scl.cell_uniq_reads[i] < minUniqReads) ||
                    ((int32_t) scl.cell_umis[i].size() < minCoveredSNPs)) {
    ...
    for (std::map<std::string, int32_t>::iterator it0 = scl.bc_map.begin(); it0 != scl.bc_map.end(); ++it0, ++ncells) {
                ...
                for (int32_t k = 0; k < nAlpha; ++k) {
                    for (int32_t l = 0; l < 3; ++l) {  // 1-Alpha
                        for (int32_t m = 0; m < 3; ++m) { // Alpha
                            double p = 0.5 * l + (m - l) * 0.5 *
                                                 gridAlpha[k]; // %A (0, 0.5a, 1.0a, 0.5-0.5a, 0.5, 0.5+0.5a, 1.0-a, 1.0-0.5a, 1.0)
        ...
        */
        ...
        if (dblBestLLK > sngBestLLK + 2) { // bestcall is doublet
            bestType = "DBL";
            bestPP = exp(
                    dblBestLLK + ((gridAlpha[dblBestAlpha] == 0.5) ? log_doublet_prior2 : log_doublet_prior1) - sumLLK);
            jBest = dBest1;
            kBest = dBest2;
            bestLLK = dblBestLLK;
